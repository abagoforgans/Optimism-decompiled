...
function sub_1a8bf244(?) payable {
    ...
    require cd[36] + (32 * ('cd', 36).length) + 36 <= calldata.size
    require msg.sender == maintainerAddress
    mem[96] = 0xa32b3cb800000000000000000000000000000000000000000000000000000000
    mem[100] = 64
    mem[164] = ('cd', 4).length
    ...
    call stor3.mem[mem[64] len 4] with:
         gas gas_remaining wei
        args mem[mem[64] + 4 len (32 * ('cd', 4).length) + (32 * ('cd', 36).length) + (32 * ('cd', 36).length) + -mem[64] + 224]
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
}
